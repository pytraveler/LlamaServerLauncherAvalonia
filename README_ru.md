...
- Каждый прогон сохраняется по профилю в директории данных (`benchmarks/<профиль>/<runId>/`: конфигурация, командная строка, лог сервера, метрики, отчёт)
- **Окно сравнения** — сравнение сохранённых прогонов бок о бок в виде Markdown-таблиц, именованные наборы сравнения, экспорт отчётов в `.md`, прикрепление дополнительных файлов к прогону
...
3. Запустите бенчмарк; после остановки сервера прогон автоматически сохраняется в профиле
4. Нажмите **Бенчмарки**, чтобы открыть окно сравнения: выберите прогоны, сравните их бок о бок в Markdown-таблицах, сохраняйте именованные наборы сравнения и экспортируйте отчёты в `.md`
