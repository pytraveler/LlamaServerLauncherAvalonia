...
- [Возможности](#возможности)
  ...
  - [Влезет ли модель в VRAM](#влезет-ли-модель-в-vram)
  - [Параметры генерации](#параметры-генерации)
  - [Спекулятивное декодирование](#спекулятивное-декодирование)
  - [Дополнительные опции](#дополнительные-опции)
  - [Определение возможностей](#определение-возможностей)
  - [Встроенная справка](#встроенная-справка)
  - [Информация о GGUF-модели](#информация-о-gguf-модели)
  - [Управление множественными инстансами сервера](#управление-множественными-инстансами-сервера)
  - [Монитор оборудования](#монитор-оборудования)
  - [Бенчмарки](#бенчмарки)
  - [MCP-серверы](#mcp-серверы)
  - [Прокси загрузки моделей по запросу (OpenAI-совместимый)](#прокси-загрузки-моделей-по-запросу-openai-совместимый)
  - [Интеграция с ComfyUI](#интеграция-с-comfyui)
  - [Сценарии](#сценарии)
  ...
  - [Системная интеграция](#системная-интеграция)
...
- [Установка](#установка)
- [Проверка релизов](#проверка-релизов)
- [Сборка из исходников](#сборка-из-исходников)
  - [Необходимые компоненты](#необходимые-компоненты)
  - [Команды сборки](#команды-сборки)
- [Тесты](#тесты)
- [Использование](#использование)
  - [Управление профилями](#управление-профилями-1)
  - [Работа со сценариями](#работа-со-сценариями)
  - [Запуск бенчмарков](#запуск-бенчмарков)
  - [Подключение MCP-серверов](#подключение-mcp-серверов)
  - [Сервер потоковой передачи логов](#сервер-потоковой-передачи-логов)
  - [Прокси загрузки моделей по запросу](#прокси-загрузки-моделей-по-запросу)
...
- **Выбор модели из списка** — Сканирование папки (при желании рекурсивное) на файлы `.gguf` с выбором из фильтруемого списка с метаданными и размером файла. В каждой строке стоит ещё и то, сколько модель попросит у карты — зелёным, оранжевым или красным, смотря влезает ли она в свободную сейчас VRAM, — а модель из нескольких файлов показывается одной строкой и размером всех своих частей сразу. Последняя папка запоминается
- **Выбор проектора из списка** — у поля MMProj свой список, в котором показаны только файлы проекторов; из списка моделей они, наоборот, убраны — запустить проектор как модель всё равно нельзя
- **Загрузчик с HuggingFace** — окно, которое ищет по Хабу, показывает кванты репозитория с размерами и качает выбранный прямо в вашу папку моделей. Принимает слова для поиска, `owner/name`, ссылку на репозиторий или ссылку на файл внутри него. Модель из нескольких файлов показывается одной строкой и размером всех частей сразу. Видны прогресс, скорость и оставшееся время; оборванное скачивание оставляет рядом с целевым файлом `.part` и продолжается с этой точки, а не с нуля. Готовый файл подставляется в поле **Модель**. Для gated- и приватных репозиториев в том же окне вводится токен на чтение
...
### Влезет ли модель в VRAM
- **Вердикт до запуска** — строка под полями вкладки **Генерация** говорит, влезает ли модель с теми значениями, что сейчас на форме, в свободную на карте память. Пересчитывается по мере ввода и приходит в трёх цветах: влезает с запасом, влезает впритык или не влезает
- **Разбивка по частям** — веса, KV-кэш, буферы вычислений, запас и визуальный проектор, если он есть, плюс сколько блоков уйдёт на карту, а сколько останется в оперативной памяти. Всё это читается из самого GGUF, вплоть до размера каждого блока в таблице тензоров
- **Три кликабельные подсказки** — максимум слоёв при текущем контексте, максимум контекста при текущем числе слоёв и, для MoE-моделей, сколько блоков экспертов оставить на CPU. Подсказка прячется, когда предлагает то, что уже выставлено
- **С учётом перезапуска** — пока сервер работает с той же моделью, занятое им считается снова свободным, а строка помечается словами «после перезапуска»: применение новых настроек сервер перезапускает, и старая память освобождается раньше, чем занимается новая
- **Сверка с реальностью** — при включённом **подробном выводе** появляется третья строка: сколько llama.cpp занял на самом деле, в той же разбивке, и на сколько оценка промахнулась; то же сравнение уходит в лог приложения при каждой загрузке модели
...
- Проектор на GPU (`--mmproj-offload`, `--no-mmproj-offload`) — в положении «Выкл» проектор остаётся в оперативной памяти и возвращает карте гигабайт-другой, ценой того, что предобработка картинок уходит на CPU
- Шаблоны Jinja (`--jinja`, `--no-jinja`) — свежие сборки включают их по умолчанию; без них не работают ни вызовы инструментов, ни MCP-серверы
...
- **Таблица тензоров** — кроме метаданных читается и таблица тензоров, так что размер каждого блока известен по отдельности; на этом и построена оценка VRAM
- **Модели из нескольких файлов** — модель, разложенная на несколько `.gguf`, читается как одна: части находятся по имени, метаданные сводятся вместе, размеры складываются
...
- Вкладка предупреждает о практических нюансах: серверы работают дочерними процессами с правами лаунчера, включение MCP ограничивает CORS до localhost, если не задать явно, а вызовы инструментов требуют jinja-шаблонов, которыми управляет переключатель **Шаблоны Jinja**
...
- Подробное логирование (`-v`, `--verbose`) — трёхсекционный переключатель на вкладке **Параметры**, хранится в профиле; свою разбивку по памяти llama.cpp печатает только при такой подробности, а с ней и сверяет свою оценку панель VRAM
...
- **Откат на заменяемую сборку** — при обновлении прежняя сборка не удаляется, а переезжает в `llama.cpp.prev` рядом с рабочим каталогом, и окно загрузки по требованию возвращает её на место — мгновенно и без сети. Откат меняет их местами, так что его самого можно откатить тем же способом; хранится ровно одна сборка, а галочка на вкладке **Обновления** возвращает прежнее поведение с удалением
- **Загрузка в служебную папку под вопросом** — если папку для скачивания выбрали внутри собственного каталога llama.cpp лаунчера, который перезаписывается при каждом обновлении, приложение сначала переспросит
...
- **Проверку обновлений можно выключить** — две галочки на вкладке **Обновления**, отдельно для лаунчера и для llama.cpp. Снятая галочка прекращает обращения к GitHub и гасит кнопку обновления, в том числе ту, что зажигалась из кэша релизов вообще без сети
...
- **Вставка строки запуска** — кнопка рядом с панелью просмотра команды берёт строку запуска из буфера обмена и открывает ту же панель разбора. Разбор рассчитан на то, что люди действительно копируют: блоки в тройных кавычках из README, переносы строк через `^`, `\` и обратную кавычку, приглашения оболочки, перенаправления и всё, что идёт после `&&`
...
Брошенные `.bat`, `.sh` и `.command` проходят через тот же разбор команды, что и вставка из буфера, и открывают ту же панель выбора.
...
- **Масштаб интерфейса** от 80 до 160 процентов — масштабирует содержимое каждого окна целиком (текст, контролы, отступы, значки и фиксированные размеры), тогда как размер шрифта меняет только текст; отдельная галочка «Менять размеры окон вместе с масштабом» заставляет окна тянуться за содержимым
...
- Простая миграция между директориями — переносится всё содержимое каталога: профили, сценарии, настройки, ротированные логи, конфигурации MCP, кэш релизов, скачанные сборки llama.cpp и та, что сохранена для отката
...
Текущее покрытие включает слой командной строки (`CommandLineParser`, `CommandLineBuilder`, `ServerConfiguration`), движок оптимизации (HPO), хелперы протокола прокси по запросу (`ProxyProtocol`), чтение GGUF-метаданных, сканирование папки моделей, парсеры метрик инференса/GPU/CPU (NVIDIA + AMD), построение endpoint/curl-сниппетов, выбор сборки llama.cpp под бэкенд, фильтрацию логов сервера, советчик по падениям, конвейер метрик/отчётов бенчмарков, MCP-конфигурацию (генерация, парсинг, валидация, встраивание в командную строку) прогон промта в бенчмарках (разбиение запросов, разбор ответов, построение расшифровки), оценку VRAM и построенный на ней план, разбор сообщений llama.cpp о занятой памяти, импорт строки запуска и сохранение сборки llama.cpp с откатом. Каждая область — отдельный файл `*Tests.cs`, подключаемый в `Program.cs`, поэтому покрытие растёт постепенно.
...
│   ├── CommandImport                  # Находит команду llama-server во вставленном тексте или скрипте
...
│   ├── ServerMemoryParser             # Читает из лога сервера, сколько llama.cpp занял на самом деле
...
│   ├── ModelScanService               # Рекурсивное сканирование папки на .gguf: модели и проекторы
│   ├── VramEstimator                  # Веса, KV-кэш и буферы конфигурации, посчитанные из GGUF
│   ├── VramPlan                       # Превращает оценку в вердикт, разбивку и подсказки
│   ├── VramComparison                 # Сводит оценку с тем, что сервер измерил
│   ├── UiScaleService                 # Масштабирует содержимое окон (layout transform, overlay-попапы)
