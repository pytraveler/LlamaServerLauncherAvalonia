...
**Paste command**, in the same menu, reads a llama-server command line from the clipboard - the way they get passed around in chats and READMEs. A command split over several lines with a trailing backslash, caret or backtick is joined back together; code fences, comments, shell prompts and a trailing `> log.txt` are ignored; a bare list of arguments with no program name in front of it works too. What it finds goes through the same review panel as a dropped file.
